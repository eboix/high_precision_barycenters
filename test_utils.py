...
def save_dense_data_representation(dense_data, fprefix):
    for i, img in enumerate(dense_data):
        imwrite(fprefix + '_' + str(i+1) + '.png', img)
...
##########################
# Compute exact cost between two sparse images, by solving the LP (Using cvxopt).
def exact_cost_glpk(sparse_datum1, sparse_datum2):
    ...
    for i in range(n1):
        for j in range(n2):
            row_vals.append(curr_num)
            col_vals.append(i)
            row_vals.append(curr_num)
            col_vals.append(n1 + j)
            b.append((sparse_datum1[i,0] - sparse_datum2[j,0])**2 + (sparse_datum1[i,1] - sparse_datum2[j,1])**2)
            curr_num += 1
    A = cvxopt.spmatrix([1]*len(row_vals), row_vals, col_vals, (curr_num, n1 + n2))
    b = cvxopt.matrix(b)
    c = cvxopt.matrix(-np.hstack((sparse_datum1[:,2]/np.sum(sparse_datum1[:,2]), sparse_datum2[:,2]/np.sum(sparse_datum2[:,2])))) # n_1 + n_2 length 

    sols = cvxopt.solvers.lp(c,A,b,solver='glpk')
    print(sols)
    assert(sols['status'] == 'optimal')

    return -sols['primal objective']

##########################
# Compute exact cost between two sparse images, by solving the LP (Using the LEMON solver).
def exact_cost_lemon(sparse_datum1, sparse_datum2):
    # sparse_datum1 and sparse_datum2 are n1 x 3 and n2 x 3 numpy arrays, respectively
    n1 = sparse_datum1.shape[0]
    n2 = sparse_datum2.shape[0]
    ...
    os.system('./lemon_solver/LemonNetworkSimplex < curr_cost_problem > curr_sol_file')
